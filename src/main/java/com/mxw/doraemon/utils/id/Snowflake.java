package com.mxw.doraemon.utils.id;
...
public class Snowflake {
	...
	public static long getDistributedGlobalNumber(String... serverIds) {

		long datacenterId = RandomUtils.nextInt(1, 31);
		long workerId = RandomUtils.nextInt(1, 31);
	...
	public static void main(String[] args) {
		...
		int max = 500;
		AtomicInteger duplicatedNum = new AtomicInteger();

		ThreadPoolProcessor threadPoolProcessor = ThreadPoolProcessor.getInstanceFixed(max);
		for (int i = 0; i < max; i++) {
			threadPoolProcessor.execute(() -> {
				Long id = getDistributedGlobalNumber();
				System.out.println(id);
				boolean isSuccess = ids.add(id);
				if (!isSuccess) {
					System.out.println("!!!id duplicate!!!");
					duplicatedNum.getAndIncrement();
				}
			});
		}
		...
		System.out.println("total number is ："+max);
		System.out.println("duplicated number is :"+duplicatedNum.toString());
	}
