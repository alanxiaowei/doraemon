...
public class RSAHelper2048{
	 ...
	 * @throws Exception
	 */
	public String signRSA(byte[] plainBytes,String charset) throws Exception {
		String SIGNATURE_ALGORITHM = "SHA1withRSA";
		Signature signature = Signature.getInstance(SIGNATURE_ALGORITHM);
		signature.initSign(localPrivKey);
		signature.update(plainBytes);
	...
	public String sign(String data) {
		...
		try {
			return signRSA(data.getBytes(Charset.forName("UTF-8")), "UTF-8");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String encrypt(String data) {
		...
		try {
			...
			outbuf = new ByteArrayOutputStream(nBlock
					* decryptBlock);
			cipher.init(Cipher.ENCRYPT_MODE, peerPubKey);
			// cryptedBase64Str =
			// Base64.encodeBase64String(cipher.doFinal(plaintext.getBytes()));
		...
		return Base64.encodeBase64String(outbuf.toByteArray());
		
	}

	public String decrypt(String cryptedBytes) {
		// 输出buffer, , 大小为nBlock个encryptBlock
		ByteArrayOutputStream outbuf = null;
		try {
			String CIPHER_ALGORITHM = "RSA/ECB/PKCS1Padding"; // 加密block需要预留11字节
			...
			// 如果是Base64编码的话，则要Base64解码
			data = Base64.decodeBase64(cryptedBytes);
			int KEYBIT = 2048;
			int RESERVEBYTES = 11;
			Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM);
			int decryptBlock = KEYBIT / 8; // 256 bytes
		...
		return new String(outbuf.toByteArray(), Charsets.UTF_8);
	}
}
