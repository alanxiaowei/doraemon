package com.mxw.doraemon.utils.secret.ecc.ecdsa.utils;
...
public class Der {
    ...
    public static ByteString encodeSequence(ByteString... encodedPieces) {
        int totalLen = 0;
        ByteString stringPieces = new ByteString(BinaryAscii.toBytes(0x30));
    ...
    public static ByteString encodeInteger(BigInteger r) {
        ...
        ByteString s = new ByteString(BinaryAscii.binaryFromHex(h));
        short num = s.getShort(0);
        if (num <= 0x7F) {
            s.insert(0, BinaryAscii.toBytes(s.length()));
            s.insert(0, BinaryAscii.toBytes(0x02));
            return s;
        }
        int length = s.length();
        s.insert(0, BinaryAscii.toBytes(0x00));
    ...
    public static ByteString encodeNumber(long n) {
        ...
        while (n != 0) {
            b128Digits.insert(0, BinaryAscii.toBytes((int) (n & 0x7f) | 0x80));
            n = n >> 7;
        }
    ...
    public static ByteString encodeOid(long... pieces) {
        ...
        body.insert(0, BinaryAscii.toBytes((int) (40 * first + second)));
        body.insert(0, encodeLength(body.length()).getBytes());
        body.insert(0, BinaryAscii.toBytes(0x06));
        return body;
    }
    ...
    public static ByteString encodeBitString(ByteString s) {
        ...
        s.insert(0, BinaryAscii.toBytes(0x03));
        return s;
    }
    ...
    public static ByteString encodeOctetString(ByteString s) {
        s.insert(0, encodeLength(s.length()).getBytes());
        s.insert(0, BinaryAscii.toBytes(0x04));
        return s;
    }
    ...
    public static ByteString encodeConstructed(long tag, ByteString value) {
        ...
        value.insert(0, BinaryAscii.toBytes((int) (0xa0 + tag)));
        return value;
    }
    ...
    public static String toPubStrByBase58(ByteString der) {
        return Base58.encode(der.getBytes());
    }

    public static String toPubStrByBase64(ByteString der) {
        return Base64.encodeBytes(der.getBytes());
    }
}