...
public class LogUtilMsg {
	...
	public static String formatLogHex(byte[] data) {
		...
		int row = (total % MAX_COL == 0) ? (total / MAX_COL)
				: (total / MAX_COL + 1);
		int strRow = (strTotal % MAX_COL == 0) ? (strTotal / MAX_COL)
				: (strTotal / MAX_COL + 1);
	...
	public static void main(String[] args) {
		String s = "0020C0 00 00 05 78 FF 63 C7 78 01 CC B0 14 2A 24 CC 05 68 AC 58 CF 80 F3 CF BF 57 16 35 8B F2 F8 8F"
				.replaceAll(" ", "");
		byte[] b = LogUtilMsg.hexStr2hexByte(s);
		System.out.println(LogUtilMsg.formatLogHex(b));
	}
